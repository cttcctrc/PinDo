...
async function runSmokeValidation({ app, mainWindow, noteWindowManager, getStore, outputDirectory }) {
  ...
  try {
    ...
    await waitFor(() => noteWindowManager?.windows?.size > 0);
    ...
    const expected = store.state.notes.filter(note => note.mode !== 'bookmark').length;
